...
eulaText.addEventListener("selectionchange", function (event) {
  event.preventDefault();
});

eulaText.addEventListener("pointerdown", function (event) {
  event.preventDefault();
});
 ...
 * This one requires us to do some data wrangling, so let's see one solution:
 */
const eulaInputCounter = document.querySelector("#eula-input-counter"); // Get the conuter

// Set some variables and a default text.
const maxInputs = eula.length + 20;
let currentInputs = 0;
eulaInputCounter.innerHTML = `${currentInputs}/${maxInputs}`;

// A function that counts our input and yells at us if we go to far (don't do that)
function incrementCounterOnInput() {
  currentInputs += 1;
  eulaInputCounter.innerHTML = `${currentInputs}/${maxInputs}`;
...
eulaTextArea.addEventListener("input", incrementCounterOnInput);